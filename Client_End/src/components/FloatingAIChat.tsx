...
// Lazy AI init — only when API key is provided
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const history = messages
        .filter(m => m.id !== "1") // filter out initial greeting if not wanted as history, or keep it. We'll map to model.
        .slice(-6)
        .map((m) => ({
      ...
      const aiResponse: Message = {
        ...
        role: "assistant",
        content: aiContent || "I couldn't process that request.",
        timestamp: new Date(),
      };
      console.log(context);
