  ...
  const handleBulkImport = async (data: any[]) => {
    ...
    for (const row of data) {
      try {
        ...
        if (!['present', 'absent'].includes(status)) throw new Error(`Invalid status ${status}`);
           ...
           <PDFExportButton
             onExport={async () => {
               ...
               const attendanceData = students.map(s => ({
                 ...
                 total: s.status ? 1 : 0,
                 percentage: s.status === 'present' ? 100 : 0
               }));
      ...
      <Dialog open={showBulkUpload} onOpenChange={setShowBulkUpload}>
        <DialogContent className="max-w-4xl">
           <DialogHeader>
             ...
             <DialogDescription>Columns: studentEmail, courseCode, date (YYYY-MM-DD), status (present/absent)</DialogDescription>
           </DialogHeader>
           <BulkUpload 
...
function StudentAttendanceView({ userId }: { userId: string }) {
  ...
  const fetchData = async () => {
    try {
      if (orgSlug === 'demo') {
        const mockTimestamp = (isoDate: string) => {
        ...
        const demoCourseStats: CourseAttendance[] = [
          { courseId: 'c1', courseName: 'Advanced Algorithms', courseCode: 'CS301', present: 17, absent: 1, late: 0, total: 18, percentage: 94 },
          { courseId: 'c2', courseName: 'Machine Learning', courseCode: 'CS302', present: 16, absent: 1, late: 0, total: 17, percentage: 94 },
          { courseId: 'c3', courseName: 'Cloud Computing', courseCode: 'CS303', present: 15, absent: 1, late: 0, total: 16, percentage: 94 },
          { courseId: 'c4', courseName: 'Software Engineering', courseCode: 'CS304', present: 14, absent: 1, late: 0, total: 15, percentage: 93 },
          { courseId: 'c5', courseName: 'Operating Systems', courseCode: 'CS305', present: 16, absent: 1, late: 0, total: 17, percentage: 94 },
          { courseId: 'c6', courseName: 'Technical Communication', courseCode: 'HS301', present: 16, absent: 1, late: 0, total: 17, percentage: 94 },
        ];

        const demoRecords = [
          { id: 'att-1', courseId: 'c1', status: 'present', date: mockTimestamp('2026-03-12T09:00:00') },
          { id: 'att-2', courseId: 'c2', status: 'present', date: mockTimestamp('2026-03-11T11:30:00') },
          { id: 'att-3', courseId: 'c3', status: 'present', date: mockTimestamp('2026-03-10T14:00:00') },
          { id: 'att-4', courseId: 'c4', status: 'present', date: mockTimestamp('2026-03-10T10:00:00') },
          { id: 'att-5', courseId: 'c1', status: 'absent', date: mockTimestamp('2026-03-08T09:00:00') },
          { id: 'att-6', courseId: 'c2', status: 'present', date: mockTimestamp('2026-03-07T11:30:00') },
          { id: 'att-7', courseId: 'c3', status: 'present', date: mockTimestamp('2026-03-06T14:00:00') },
          { id: 'att-8', courseId: 'c4', status: 'present', date: mockTimestamp('2026-03-05T10:00:00') },
          { id: 'att-9', courseId: 'c1', status: 'present', date: mockTimestamp('2026-03-03T09:00:00') },
          { id: 'att-10', courseId: 'c2', status: 'absent', date: mockTimestamp('2026-03-02T11:30:00') },
          { id: 'att-11', courseId: 'c5', status: 'present', date: mockTimestamp('2026-02-28T13:00:00') },
          { id: 'att-12', courseId: 'c6', status: 'present', date: mockTimestamp('2026-02-27T15:00:00') },
        ];

        const demoPresent = demoCourseStats.reduce((sum, c) => sum + c.present, 0);
        const demoTotal = demoCourseStats.reduce((sum, c) => sum + c.total, 0);

        setAttendanceRecords(demoRecords);
        setCourseAttendance(demoCourseStats);
        setOverallPercentage(Math.round((demoPresent / demoTotal) * 100));
        return;
      }
  ...
  const getLeavesLeft = (present: number, total: number) => {
    const leaves = Math.floor((present - 0.75 * total) / 0.75);
    return Math.max(0, leaves);
  };

  const getClassesNeededFor75 = (present: number, total: number) => {
    return Math.max(0, Math.ceil(3 * total - 4 * present));
  };
         ...
         {courseAttendance.map(c => (
           <div key={c.courseId} className="card-elevated p-4">
             ...
             <div className="mt-2 text-xs">
               {c.percentage >= 75 ? (
                 <span className="text-success">Leaves Left: {getLeavesLeft(c.present, c.total)}</span>
               ) : (
                 <span className="text-warning">Need {getClassesNeededFor75(c.present, c.total)} consecutive presents to reach 75%</span>
               )}
             </div>
           </div>
         ))}
             ...
             filteredRecords.sort((a,b)=>(b.date?.seconds||0) - (a.date?.seconds||0)).map(r => (
               <div key={r.id} className="flex justify-between items-center p-3 border rounded">
                 <div className="flex gap-3 items-center">
                   {r.status === 'present' ? <CheckCircle className="text-success w-5 h-5" /> : <XCircle className="text-destructive w-5 h-5" />}
                   <div>
