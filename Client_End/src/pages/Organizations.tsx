...
export default function Organizations() {
  ...
  const fetchOrganizations = async () => {
    ...
    const mockOrganizations: Organization[] = [
      ...
      },
      {
        ...
        email: 'contact@nti.edu',
        phone: '+91 172 5550123',
        website: 'https://nti.example.edu',
        status: 'pending',
        verified: false,
        totalUsers: 0,
        totalStudents: 0,
  ...
  const handleVerify = (id: string, verified: boolean) => {
    setOrganizations((prev) =>
      prev.map((org) =>
        org.id === id ? { ...org, verified, updatedAt: new Date() } : org,
      ),
    );
  ...
  const handleApproveOrganization = (id: string) => {
    setOrganizations((prev) =>
      prev.map((org) =>
        org.id === id
          ? { ...org, status: 'active', verified: true, updatedAt: new Date() }
          : org,
      ),
    );
  ...
  const handleRejectOrganization = (id: string) => {
    setOrganizations((prev) =>
      prev.map((org) =>
        org.id === id
          ? { ...org, status: 'suspended', verified: false, updatedAt: new Date() }
          : org,
      ),
    );
