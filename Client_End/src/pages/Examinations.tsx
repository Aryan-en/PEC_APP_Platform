...
export default function Examinations() {
  ...
  return <StudentExaminationsView userId={user.uid} orgSlug={orgSlug} />;
}
...
function StudentExaminationsView({ userId, orgSlug }: { userId: string, orgSlug?: string }) {
  const [loading, setLoading] = useState(true);
  ...
  const fetchData = async () => {
    if (orgSlug === 'demo') {
      // ── DEMO DATA ──
      setUserData({ fullName: 'Aryan Singh', enrollmentNumber: '25111001' });
      
      const demoSemesters = [
        {
          semester: 2, academicYear: '2025-2026', sgpa: 8.85, totalCredits: 22, totalPoints: 194.7,
          grades: [
            { id: 'g1', courseCode: 'CH102', courseName: 'Thermodynamics', credits: 4, totalMarks: 85, grade: 'A', gradePoints: 9 },
            { id: 'g2', courseCode: 'CH104', courseName: 'Fluid Mechanics', credits: 4, totalMarks: 91, grade: 'A+', gradePoints: 10 },
            { id: 'g3', courseCode: 'MA102', courseName: 'Mathematics II', credits: 4, totalMarks: 78, grade: 'B+', gradePoints: 8 },
            { id: 'g4', courseCode: 'PH102', courseName: 'Physics II', credits: 4, totalMarks: 82, grade: 'A', gradePoints: 9 },
            { id: 'g5', courseCode: 'CH106L', courseName: 'Fluid Mechanics Lab', credits: 2, totalMarks: 88, grade: 'A', gradePoints: 9 },
          ]
        },
        {
          semester: 1, academicYear: '2025-2026', sgpa: 8.60, totalCredits: 20, totalPoints: 172,
          grades: [
            { id: 'g6', courseCode: 'CH101', courseName: 'Process Calculations', credits: 4, totalMarks: 80, grade: 'A', gradePoints: 9 },
            { id: 'g7', courseCode: 'MA101', courseName: 'Mathematics I', credits: 4, totalMarks: 75, grade: 'B+', gradePoints: 8 },
            { id: 'g8', courseCode: 'PH101', courseName: 'Physics I', credits: 4, totalMarks: 88, grade: 'A', gradePoints: 9 },
            { id: 'g9', courseCode: 'CS101', courseName: 'Programming in C', credits: 4, totalMarks: 84, grade: 'A', gradePoints: 9 },
            { id: 'g10', courseCode: 'CS101L', courseName: 'Programming Lab', credits: 2, totalMarks: 72, grade: 'B', gradePoints: 7 },
            { id: 'g11', courseCode: 'ME101', courseName: 'Engineering Graphics', credits: 2, totalMarks: 85, grade: 'A', gradePoints: 9 },
          ]
        }
      ];
      setSemesters(demoSemesters);
      setExpandedSemester(2);
      setCgpa(8.73);
      setTotalCredits(42);

      // Future/Upcoming exams based on current date
      setSchedules([
        { id: 's1', courseName: 'Thermodynamics', courseCode: 'CH102', date: '2026-03-20', startTime: '10:00 AM', endTime: '11:00 AM', room: 'LH-101', type: 'Quiz' },
        { id: 's2', courseName: 'Mathematics II', courseCode: 'MA102', date: '2026-04-05', startTime: '09:00 AM', endTime: '11:00 AM', room: 'Exam Hall A', type: 'Midsem' },
        { id: 's3', courseName: 'Fluid Mechanics', courseCode: 'CH104', date: '2026-04-10', startTime: '02:00 PM', endTime: '04:00 PM', room: 'Exam Hall B', type: 'Midsem' },
        { id: 's4', courseName: 'Fluid Mechanics Lab', courseCode: 'CH106L', date: '2026-05-15', startTime: '09:00 AM', endTime: '12:00 PM', room: 'Fluid Lab', type: 'Practical' },
        { id: 's5', courseName: 'Physics II', courseCode: 'PH102', date: '2026-05-18', startTime: '10:00 AM', endTime: '01:00 PM', room: 'Exam Hall A', type: 'Endsem' },
        { id: 's6', courseName: 'Software Engineering Mini Project', courseCode: 'CS304', date: '2026-05-25', startTime: '09:00 AM', endTime: '04:00 PM', room: 'CS Conference Room', type: 'Viva' },
      ]);
      setLoading(false);
      return;
    }
        ...
        <TabsContent value="schedule" className="space-y-4">
           {schedules.length === 0 ? <p className="text-center text-muted-foreground p-8">No upcoming exams</p> : (
             <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {schedules.map(s => {
                  let badgeColors = "bg-primary text-primary-foreground";
                  const type = s.type?.toLowerCase() || '';
                  if (type.includes('quiz')) badgeColors = "bg-slate-500 hover:bg-slate-600 text-white border-0";
                  else if (type.includes('midsem')) badgeColors = "bg-orange-500 hover:bg-orange-600 text-white border-0";
                  else if (type.includes('endsem') || type.includes('final')) badgeColors = "bg-red-500 hover:bg-red-600 text-white border-0";
                  else if (type.includes('practical') || type.includes('lab')) badgeColors = "bg-blue-500 hover:bg-blue-600 text-white border-0";
                  else if (type.includes('viva')) badgeColors = "bg-green-600 hover:bg-green-700 text-white border-0";

                  // Extract color generic variant from badgeColors for left-border
                  let borderLeftColor = "border-l-primary";
                  if (type.includes('quiz')) borderLeftColor = "border-l-slate-500";
                  if (type.includes('midsem')) borderLeftColor = "border-l-orange-500";
                  if (type.includes('endsem') || type.includes('final')) borderLeftColor = "border-l-red-500";
                  if (type.includes('practical') || type.includes('lab')) borderLeftColor = "border-l-blue-500";
                  if (type.includes('viva')) borderLeftColor = "border-l-green-600";

                  return (
                    <div key={s.id} className={cn("card-elevated p-5 flex flex-col justify-between border-l-4 h-full", borderLeftColor)}>
                       <div className="flex justify-between items-start mb-3 gap-2">
                          <h3 className="font-semibold text-foreground line-clamp-2 leading-tight">{s.courseName}</h3>
                          <Badge className={cn("shrink-0 uppercase tracking-wider text-[10px]", badgeColors)}>{s.type}</Badge>
                       </div>
                       <div>
                          <p className="text-xs font-medium text-muted-foreground mb-3">{s.courseCode}</p>
                          <div className="flex flex-col gap-2 bg-muted/30 rounded-lg p-3 text-[13px] text-muted-foreground">
                             <span className="flex items-center gap-2"><Calendar className="w-4 h-4 text-foreground/50"/> {s.date?.toDate?.().toLocaleDateString() || s.date}</span>
                             <span className="flex items-center gap-2"><Clock className="w-4 h-4 text-foreground/50"/> {s.startTime} - {s.endTime}</span>
                             {s.room && <span className="flex items-center gap-2"><Shield className="w-4 h-4 text-foreground/50"/> {s.room}</span>}
                          </div>
                       </div>
                    </div>
                  );
