...
export default function AuthEnhanced() {
  ...
  // Mock: skip auth check, user is never pre-authenticated in mock mode
  useEffect(() => {
    // No Firebase auth listener — do nothing
  }, [navigate]);
  ...
  const handleGoogleAuth = async (isSignUp: boolean) => {
    ...
    try {
      setLoading(true);
      // Mock Google auth — simulate success
      await new Promise(resolve => setTimeout(resolve, 800));
  ...
  return (
    <div className="min-h-screen bg-muted/30 flex items-center justify-center p-4">
      ...
      >
        <Card>
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">Welcome to PEC App</CardTitle>
