...
export default function Departments() {
    ...
    const mockDepartments = [
      {
        ...
        hod: 'Dr. Priya Mehta',
        status: 'active',
        description: 'Software, systems, and AI studies.',
      },
      {
        id: 'd-2',
        code: 'ECE',
        ...
        hod: 'Dr. Aman Verma',
        status: 'active',
        description: 'Electronics, communication, and embedded design.',
      },
    ];
  ...
  const handleBulkImport = async (rows: any[]) => {
    ...
    const valid = prepared.filter((r) => r.name && r.code);
    const failed = prepared.length - valid.length;
    if (valid.length > 0) {
      setDepartments((prev) => [...valid, ...prev]);
    }
    ...
    return {
      ...
      errors:
        failed > 0
          ? ['Some rows were skipped because name/code were missing']
          : [],
    };
  };
