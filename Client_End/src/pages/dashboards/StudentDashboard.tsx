...
export function StudentDashboard() {
  ...
  useEffect(() => {
    if (loading) return;
    if (location.hash !== '#pec-news') return;
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.section
        id="pec-news"
        variants={item}
        className="card-elevated p-6 scroll-mt-28"
      >
        <div className="flex items-center justify-between mb-5 gap-3 flex-wrap">
          <div>
            <h2 className="text-lg font-semibold text-foreground">PEC News & Announcements</h2>
            <p className="text-sm text-muted-foreground">Latest event updates and achievements from PEC Chandigarh.</p>
          </div>
          <Badge variant="outline" className="bg-primary/10 border-primary/30 text-primary">Live Updates</Badge>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {[
            {
              type: 'Event Update',
              title: 'Innovation Conclave 2026 Opens Next Week',
              description: 'Registrations are now live for the annual innovation conclave featuring startup showcases and keynote sessions by PEC alumni founders.'
            },
            {
              type: 'Achievement',
              title: 'PEC Team Wins National Robotics Challenge',
              description: 'The Department of Mechanical Engineering secured first place among 120 colleges with an autonomous rescue-bot prototype.'
            },
            {
              type: 'Announcement',
              title: 'Campus Placement Drive by Top Product Firms',
              description: 'New hiring slots have been announced for final-year students across software, analytics, and core engineering roles.'
            },
            {
              type: 'Event Update',
              title: 'Research Symposium on Sustainable Infrastructure',
              description: 'Civil and environmental research groups will present ongoing projects focused on smart and climate-resilient campus systems.'
            },
            {
              type: 'Achievement',
              title: 'PEC Students Publish in IEEE Conference',
              description: 'Three final-year student papers on edge computing and secure IoT were accepted at an international IEEE conference.'
            },
            {
              type: 'Announcement',
              title: 'HackPEC 48-Hour Build Marathon Announced',
              description: 'Interdisciplinary teams can apply now for HackPEC, with mentorship tracks in AI, embedded systems, and social innovation.'
            },
          ].map((news, index) => (
            <article key={`${news.title}-${index}`} className="rounded-xl border border-border bg-card/50 p-4 hover:bg-accent/20 transition-colors">
              <Badge variant="secondary" className="mb-3">{news.type}</Badge>
              <h3 className="text-sm font-semibold text-foreground leading-snug">{news.title}</h3>
              <p className="text-sm text-muted-foreground mt-2 leading-relaxed">{news.description}</p>
            </article>
          ))}
        </div>
      </motion.section>
