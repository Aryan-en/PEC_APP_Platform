...
export function CollegeAdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (mockUser.role !== 'college_admin' && mockUser.role !== 'super_admin') {
      toast.error("Access denied");
      navigate('/dashboard');
      return;
    }

    fetchDashboardData(mockUser.organizationId);
    setLoading(false);
  }, [mockUser, authLoading, navigate]);

  const fetchDashboardData = (orgId?: string) => {
    try {
      // Mock Users
      const studentCount = 1250;
      const facultyCount = 85;

      // Mock Courses
      const coursesCount = 42;
      ...
      // Mock Departments
      setDepartments([
        { name: 'Computer Science', students: 450, faculty: 30, attendance: 88 },
        { name: 'Mechanical', students: 300, faculty: 25, attendance: 82 },
        { name: 'Electrical', students: 280, faculty: 20, attendance: 85 },
      ]);

      // Mock Recent Admissions
      setRecentAdmissions([
        { id: '1', fullName: 'Rahul Kumar', department: 'Computer Science', createdAt: { seconds: Date.now() / 1000 - 86400 }, status: 'active' },
        { id: '2', fullName: 'Priya Singh', department: 'Electrical', createdAt: { seconds: Date.now() / 1000 - 172800 }, status: 'pending' },
      ]);
