            ...
            {activeTab === 'Attendance' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {[
                    { id: 'CS201', name: 'Data Structures', attended: 35, total: 40 },
                    { id: 'CS202', name: 'Web Development', attended: 20, total: 30 },
                    { id: 'CS203', name: 'Database Systems', attended: 28, total: 32 },
                    { id: 'CS204', name: 'Computer Networks', attended: 40, total: 42 },
                    { id: 'MA201', name: 'Engineering Mathematics', attended: 15, total: 25 },
                  ].map((subject, i) => {
                    const missed = subject.total - subject.attended;
                    const percentage = Math.round((subject.attended / subject.total) * 100);
                    const isSafe = percentage >= 75;
                    const safeLeaves = Math.floor((subject.attended - 0.75 * subject.total) / 0.75);
                    const classesToAttend = Math.ceil(3 * subject.total - 4 * subject.attended);
                    ...
                    return (
                      <div key={i} className="bg-[#1a1a1a] p-6 rounded-xl flex flex-col justify-between shadow-lg">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">{subject.id}</p>
                            <h3 className="font-semibold text-lg text-white leading-tight min-h-[50px]">{subject.name}</h3>
                          </div>

                          {/* Circular Progress Display matching design */}
                          <div className="relative w-[100px] h-[100px] flex-shrink-0 flex items-center justify-center">
                            <svg className="w-full h-full transform -rotate-90">
                              ...
                              />
                              <circle
                                className={isSafe ? "text-[#10b981]" : "text-[#ef4444]"}
                                strokeWidth="8"
                                strokeLinecap="round"
                                stroke="currentColor"
                                fill="transparent"
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center pt-1">
                              <span className="text-2xl font-bold leading-none tracking-tight text-white">{percentage}%</span>
                              <span className="text-[9px] uppercase tracking-wider text-muted-foreground mt-1">Present</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-8">
                          ...
                          <div className="flex justify-between text-sm py-2 border-b border-white/5">
                            ...
                            <span className="font-bold text-white">{missed}</span>
                          </div>
                          <div className="flex justify-between text-sm py-2">
                            <span className="text-muted-foreground">Total Classes</span>
                            <span className="font-bold text-white">{subject.total}</span>
                          </div>

                          <div className={`mt-4 w-full p-3 rounded-md text-xs font-medium text-center ${isSafe ? 'bg-destructive/10 text-destructive' : 'bg-destructive/10 text-destructive'}`}>
                            {isSafe
                              ? `You can safely miss ${safeLeaves} more class${safeLeaves !== 1 ? 'es' : ''}`
                              : `Must attend next ${classesToAttend} class${classesToAttend !== 1 ? 'es' : ''} to reach 75%`
                            }
                          </div>
                        </div>
                      </div>
