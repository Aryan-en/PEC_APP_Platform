...
export default function Faculty() {
    ...
    try {
      ...
      const facultyWithProfiles = await Promise.all(
        facultyUsers.map(async (user) => {
          try {
            const profileDoc = await getDoc(doc(db, 'facultyProfiles', user.id as string));
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateDoc(doc(db, 'users', editingFaculty.id as string), {
        fullName: facultyForm.fullName,
        email: facultyForm.email,
        updatedAt: serverTimestamp(),
      });

      await updateDoc(doc(db, 'facultyProfiles', editingFaculty.id as string), {
        ...facultyForm,
        updatedAt: serverTimestamp(),
      });
  ...
  const handleDelete = async (facultyId: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'users', facultyId as string));
      await deleteDoc(doc(db, 'facultyProfiles', facultyId as string));
  ...
  const promoteToHOD = async (fac: any) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'facultyProfiles', fac.id as string), {
      ...
      if (!deptSnap.empty) {
        ...
        await updateDoc(doc(db, 'departments', deptDoc.id as string), {
          hodId: fac.id,
  ...
  const promoteToPlacementOfficer = async (fac: any) => {
    try {
      await updateDoc(doc(db, 'users', fac.id as string), {
