...
const bankDetails = {
  ...
  accountName: 'PEC App Education Trust',
...
export default function PaymentDetail() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch invoice
        const docRef = doc(db, 'feeRecords', id as string);
        ...
        const settingsRef = doc(db, 'paymentSettings', 'admin_config' as string);
    ...
    fetchData();
  }, [id, authLoading, user, isAdmin, router.push]);
  ...
  const pollPaymentStatus = (invoiceId: string, maxAttempts: number, attempt: number = 0) => {
    ...
    const pollTimeout = setTimeout(async () => {
      try {
        const docRef = doc(db, 'feeRecords', invoiceId as string);
        const docSnap = await getDoc(docRef);
