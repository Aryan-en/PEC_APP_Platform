...
export default function Organizations() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingOrg) {
        ...
        await updateDoc(doc(db, 'organizations', editingOrg.id as string), {
          ...formData,
  ...
  const handleDeleteOrganization = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'organizations', id as string));
  ...
  const handleApproveOrganization = async (id: string) => {
    try {
      await updateDoc(doc(db, 'organizations', id as string), {
        status: 'active',
        verified: true,
        updatedAt: serverTimestamp(),
      });
  ...
  const handleVerify = async (orgId: string, verified: boolean) => {
    try {
      await updateDoc(doc(db, 'organizations', orgId as string), {
        verified,
