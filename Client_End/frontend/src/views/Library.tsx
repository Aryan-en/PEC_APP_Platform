...
export default function Library() {
  ...
  const handleUpdateBook = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'books', editingBook.id as string), updateData);
  ...
  const handleReturnBook = async (borrowId: string) => {
    try {
      ...
      await updateDoc(doc(db, 'bookBorrows', borrowId as string), {
        returnDate: new Date(),
        status: 'returned',
      });
      ...
      if (book) {
        await updateDoc(doc(db, 'books', borrow.bookId as string), {
          availableCopies: book.availableCopies + 1,
        });
      }
