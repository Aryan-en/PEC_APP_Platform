...
import {  collection, doc, getDocs, updateDoc, deleteDoc, query, where, limit, serverTimestamp , writeBatch } from 'firebase/firestore';
...
export default function Users() {
  ...
  useEffect(() => {
    ...
    loadUsers();
  }, [user, isAdmin, isFaculty, isPlacementOfficer, router.push, authLoading, orgSlug]);
  ...
  const handleCreate = async () => {
    try {
      ...
      if (userForm.role === 'student') {
        const linkedProfileRef = doc(db, 'studentProfiles', userRef.id as string);
        ...
        batch.set(linkedProfileRef, {
         ...
         const linkedProfileRef = doc(db, 'facultyProfiles', userRef.id as string);
         batch.set(linkedProfileRef, {
          uid: userRef.id,
          fullName: userForm.fullName,
          email: userForm.email,
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateDoc(doc(db, 'users', editingUser.id as string), {
        fullName: userForm.fullName,
        email: userForm.email,
        role: userForm.role,
        updatedAt: serverTimestamp(),
    ...
    for (let i = 0; i < data.length; i += CHUNK_SIZE) {
        ...
        chunk.forEach((row) => {
            ...
            if (row.role === 'student' || !row.role) {
                const profileRef = doc(db, 'studentProfiles', userRef.id as string);
                batch.set(profileRef, {
                 ...
                 const profileRef = doc(db, 'facultyProfiles', userRef.id as string);
                 batch.set(profileRef, {
                    uid: userRef.id,
                    fullName: row.fullName || row.name,
                    email: row.email,
