...
import {  collection, doc, updateDoc, query, where, orderBy, serverTimestamp , onSnapshot, arrayUnion } from 'firebase/firestore';
...
export default function HostelAdmin() {
  ...
  const updateStatus = async (issueId: string, newStatus: string) => {
    try {
      await updateDoc(doc(db, 'hostelIssues', issueId as string), {
        status: newStatus,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await updateDoc(doc(db, 'hostelIssues', selectedIssue.id as string), {
