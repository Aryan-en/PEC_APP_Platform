    ...
    try {
      ...
      const studentData = await Promise.all(
        enrolledStudents.map(async (en) => {
          const userDoc = await getDoc(doc(db, 'users', en.studentId as string));
          return {
            id: en.studentId,
  ...
  const handleSave = async () => {
    try {
      const batchPromises = students.map(async (student) => {
        ...
        if (student.recordId) {
           await updateDoc(doc(db, 'attendance', student.recordId as string), data);
        } else {
  ...
  const handleBulkImport = async (data: any[]) => {
    ...
    for (const row of data) {
      try {
        ...
        if (!attSnap.empty) {
           await updateDoc(doc(db, 'attendance', attSnap.docs[0].id as string), recordData);
        } else {
  ...
  const fetchData = async () => {
    try {
      ...
      for (const d of enrollSnap.docs) {
        const en = d.data();
        const courseDoc = await getDoc(doc(db, 'courses', en.courseId as string));
