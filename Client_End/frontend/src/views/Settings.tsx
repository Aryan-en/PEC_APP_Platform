    ...
    const loadUserData = async () => {
      try {
        ...
        if (userInfo?.role === 'student') {
          const profileDoc = await getDoc(doc(db, 'studentProfiles', user?.uid as string));
          setProfileData(profileDoc.data());
        } else if (userInfo?.role === 'faculty') {
          const profileDoc = await getDoc(doc(db, 'facultyProfiles', user?.uid as string));
          setProfileData(profileDoc.data());
        } else if (userInfo?.role === 'college_admin') {
          const profileDoc = await getDoc(doc(db, 'collegeAdminProfiles', user?.uid as string));
          setProfileData(profileDoc.data());
        } else if (userInfo?.role === 'placement_officer') {
          const profileDoc = await getDoc(doc(db, 'placementOfficerProfiles', user?.uid as string));
          setProfileData(profileDoc.data());
        } else if (userInfo?.role === 'recruiter') {
          const profileDoc = await getDoc(doc(db, 'recruiterProfiles', user?.uid as string));
          setProfileData(profileDoc.data());
        }
      } catch (error) {
