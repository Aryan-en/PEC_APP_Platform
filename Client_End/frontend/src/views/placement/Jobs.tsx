...
import {  collection, doc, getDoc, getDocs, addDoc, setDoc, updateDoc, deleteDoc, query, where, orderBy, serverTimestamp , onSnapshot, Timestamp } from 'firebase/firestore';
...
export default function Jobs() {
  ...
  useEffect(() => {
    ...
    const checkUserProfile = async () => {
      ...
      try {
        const profileRef = doc(db, 'placementProfiles', user?.uid as string);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingJob) {
        await updateDoc(doc(db, 'jobs', editingJob.id as string), data);
  ...
  const processApplication = async () => {
    ...
    if (!useExistingResume || !hasResume) {
        ...
        try {
           ...
           // Mock Upload URL
           resumeUrlToUse = `https://firebasestorage.googleapis.com/v0/b/PEC App-demo/o/resumes%2F${user?.uid}_${Date.now()}.pdf?alt=media`;
           
           // If they chose to make public, update their profile
           if (isResumePublic) {
               await updateDoc(doc(db, 'placementProfiles', user?.uid as string), {
    ...
    try {
      ...
      await setDoc(doc(db, 'job_applications', applicationId as string), {
        id: applicationId,
