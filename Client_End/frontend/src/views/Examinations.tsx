...
function ExaminationsManager({ userId, userRole }: { userId: string, userRole: string }) {
  ...
  const handleDeleteSchedule = async (id: string) => {
    ...
    await deleteDoc(doc(db, 'examSchedules', id as string));
  ...
  const fetchStudentsForGrade = async () => {
    ...
    try {
      ...
      const users = await Promise.all(enrolled.map(async (e) => {
        const u = await getDoc(doc(db, 'users', e.sid as string));
  ...
  const handleSaveGrades = async () => {
    try {
      ...
      const batchPromises = students.map(async (s) => {
        ...
        if (gd.id) {
          await updateDoc(doc(db, 'grades', gd.id as string), data);
        } else {
    ...
    if (activeTab === 'schedule') {
      ...
      for (const row of data) {
        try {
          ...
          if (!gSnap.empty) {
             await updateDoc(doc(db, 'grades', gSnap.docs[0].id as string), gradeData);
          } else {
...
function StudentExaminationsView({ userId }: { userId: string }) {
  ...
  const fetchData = async () => {
    try {
      const uDoc = await getDoc(doc(db, 'users', userId as string));
