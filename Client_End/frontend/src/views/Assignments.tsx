...
function AssignmentsManager({ userId, userRole }: { userId: string; userRole: string }) {
    ...
    try {
      ...
      const submissionsData = await Promise.all(
        submissionsSnap.docs.map(async (subDoc) => {
          ...
          const studentDoc = await getDoc(doc(db, 'users', subData.studentId as string));
  ...
  const handleUpdateAssignment = async () => {
    ...
    try {
      await updateDoc(doc(db, 'assignments', editingAssignment.id as string), {
  ...
  const handleGradeSubmission = async (submissionId: string, marks: number, feedback: string) => {
    try {
      await updateDoc(doc(db, 'submissions', submissionId as string), {
        marksObtained: marks,
        feedback,
        gradedBy: userId,
    ...
    try {
      ...
      for (const assignDoc of assignmentsSnapshot.docs) {
        ...
        if (enrolledCourseIds.includes(assignData.courseId)) {
          const courseDoc = await getDoc(doc(db, 'courses', assignData.courseId as string));
