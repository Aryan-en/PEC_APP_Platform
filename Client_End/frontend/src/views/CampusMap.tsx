  ...
  const saveRegion = async () => {
    ...
    try {
      ...
      if (editingRegion.id && !editingRegion.id.startsWith('default-')) {
        ...
        await updateDoc(doc(db, 'campusMapRegions', editingRegion.id as string), regionData);
  ...
  const saveAllChanges = async () => {
    ...
    try {
      ...
      for (const docSnap of existingRegions.docs) {
        await deleteDoc(doc(db, 'campusMapRegions', docSnap.id as string));
      }
      for (const docSnap of existingRoads.docs) {
        await deleteDoc(doc(db, 'campusMapRoads', docSnap.id as string));
      }
  ...
  const deleteRoad = async (id: string) => {
    try {
      if (!id.startsWith('road-')) {
        await deleteDoc(doc(db, 'campusMapRoads', id as string));
      }
  ...
  const deleteRegion = async (id: string) => {
    try {
      if (!id.startsWith('default-')) {
        await deleteDoc(doc(db, 'campusMapRegions', id as string));
      }
