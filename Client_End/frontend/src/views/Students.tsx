...
export default function Students() {
  ...
  useEffect(() => {
    ...
    loadStudents();
  }, [authLoading, user, isAdmin, router.push]);
    ...
    try {
      ...
      const studentsWithProfiles = await Promise.all(
        studentUsers.map(async (student) => {
          const profileDoc = await getDoc(doc(db, 'studentProfiles', student.id as string));
          return {
            ...student,
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateDoc(doc(db, 'users', editingStudent.id as string), {
        fullName: studentForm.fullName,
        email: studentForm.email,
        updatedAt: serverTimestamp(),
      });

      await updateDoc(doc(db, 'studentProfiles', editingStudent.id as string), {
        ...studentForm,
        updatedAt: serverTimestamp(),
      });
  ...
  const handleDelete = async (studentId: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'users', studentId as string));
      await deleteDoc(doc(db, 'studentProfiles', studentId as string));
