...
export default function Courses() {
  ...
  const handleUpdateCourse = async () => {
    ...
    try {
      await updateDoc(doc(db, 'courses', editingCourse.id as string), {
        ...courseForm,
        schedule: scheduleSlots.filter(s => s.day && s.startTime && s.endTime),
        updatedAt: serverTimestamp(),
      });
  ...
  const handleDeleteCourse = async (courseId: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'courses', courseId as string));
  ...
  const handleEnroll = async (course: Course) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'courses', course.id as string), {
  ...
  const handleDrop = async (courseId: string) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'enrollments', enrollmentDoc.id as string), {
        status: 'dropped',
      });
