...
// Lazy Gemini init — only when API key is provided
const geminiApiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY;
const genAI = geminiApiKey ? new GoogleGenerativeAI(geminiApiKey) : null;

const SYSTEM_PROMPT = `
You are the PEC App AI Assistant. 
...
your only answer about the details given below
            ...
            To ensure the bot acts as a specialized PEC App Knowledge Expert, this prompt focuses on the application's functional map and capabilities while strictly maintaining boundaries regarding user data.
System Prompt: PEC App System Guide

Role: You are the PEC App System Guide. Your sole purpose is to explain the features, navigation, and capabilities of the PEC App ERP platform to visitors and users.
...
Interaction Protocol:
    ...
    Direct Navigation: Use clear directions. (e.g., "To update your branding, an Admin should router.push to College Settings via the Admin Dashboard.")
...
const FloatingAIChat = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I'm your **PEC App AI Assistant**. How can I help you today?",
      timestamp: new Date(),
    },
  ]);
  ...
  async function getStudentContext(id: string) {
    if (!id) return;
    // Mock context — no Firebase
    setcontext({
      profile: { name: user.user?.fullName || 'Demo Student', department: 'Computer Science', semester: 6 },
      attendance: [],
      grades: [],
      feeRecords: [],
      timetable: [],
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!genAI) {
        throw new Error('Gemini AI assistant not configured');
      }
          ...
          >
            <div className="p-4 border-b flex items-center justify-between bg-background/50 backdrop-blur-md">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-bold">PEC App AI</h2>
