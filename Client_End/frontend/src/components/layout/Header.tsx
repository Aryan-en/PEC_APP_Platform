...
import { Bell, Search, User, ChevronDown, LogOut, Settings, HelpCircle, Menu, Building2, GraduationCap, Briefcase, Map, Users, LayoutDashboard, UtensilsCrossed, Shield } from 'lucide-react';
    ...
    <div className="fixed top-0 left-0 right-0 z-[60] flex flex-col w-full shadow-md">
      ...
      <header className="h-[64px] bg-card border-b border-border transition-all duration-300 w-full flex items-center px-4 md:px-8 justify-between relative z-[65]">
        ...
        <div className="flex items-center">
          ...
          {/* PEC APP Config Logo (Replacing PEC App regular layout) */}
          <div className="bg-primary text-white h-10 px-5 rounded-sm flex items-center justify-center text-xl tracking-[0.15em] mr-8 select-none cursor-pointer hover:bg-primary/90 transition-colors whitespace-nowrap whitespace-pre" onClick={() => router.push('/dashboard')}>
          ...
          <div className="hidden lg:flex items-center gap-2 relative z-[100]">
            {navGroups.map((group, idx) => {
              // Only process items that this user has access to
                ...
                <DropdownMenu key={idx}>
                  ...
                  <DropdownMenuContent align="start" className="w-[200px] z-[100]" sideOffset={8}>
                    {accessibleItems.map((item, itemIdx) => (
                      <DropdownMenuItem key={itemIdx} onClick={() => router.push(item.path)} className="cursor-pointer font-medium text-[13px] py-2.5">
        ...
        <div className="flex items-center gap-3 md:gap-4 ml-auto">
          ...
          <div className="relative hidden md:block w-56 group pointer-events-auto">
            <CommandMenu router.push={router.push} />
          </div>
          ...
          {/* Crest Logo Spacer Component like PEC APP */}
...
function CommandMenu({ router.push }: { router.push: any }) {
